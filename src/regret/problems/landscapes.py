...
class NKLandscape(Problem):
    ...
    def __init__(self, n: int, k: int = 2, seed: int | None = None):
        ...
        Args:
            n: Number of bits in the search space.
            k: Number of interacting neighbors (capped at n-1).
    ...
    def _initialize_landscape(self, n: int):
        """Generate random fitness contributions for each locus.
    ...
    def evaluate(self, x: np.ndarray) -> float:
        """Compute normalized NK fitness for a candidate.

        Args:
            x: Binary vector representing the candidate solution.

        Returns:
            Normalized fitness value in [0, 1].
        """
        fitness = 0.0
    ...
    def get_optimum_value(self) -> float:
        """Return exact optimum for small n or a heuristic upper bound.

        Returns:
            Exact optimum via exhaustive search when n <= 20, otherwise a
            heuristic bound assuming maximal contributions.
        """
